    ...
    public void testReadAndWriteAllTypes() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try {
                ...
                addRowsToWalAndApplyToTable(tableName, tableCopyName, rowsToInsertTotal, tsIncrement, ts, ts, rnd, engine.getWalWriter(sqlExecutionContext.getCairoSecurityContext(), tableName), true);
    ...
    public void testReadAndWriteFrom2Walls() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (
                ...
                addRowsToWalAndApplyToTable(tableName, tableCopyName, rowsToInsertTotal, tsIncrement, ts, ts, rnd, walWriter2, false);
                TestUtils.assertSqlCursors(compiler, sqlExecutionContext,  tableCopyName, tableName, LOG);
